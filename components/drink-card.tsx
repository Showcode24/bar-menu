...
export default function DrinkCard({
  ...
  useEffect(() => {
    if (isSelected && cardRef.current) {
      ...
      }, 3000) // Increased from 2000 to 3000 for better visibility
  ...
  useEffect(() => {
    if (inView && drink.image) {
      preloadImage(drink.image)
        ...
        .catch(() => setIsImageLoaded(true)) // Still mark as loaded even on error
    }
  }, [inView, drink.image])
    ...
    <>
      <motion.div
        ref={(node) => {
          // Combine refs
          if (node) {
            // @ts-ignore - this is a valid way to assign multiple refs
      ...
      {/* Full screen drink details modal */}
      {isDetailsOpen && (
